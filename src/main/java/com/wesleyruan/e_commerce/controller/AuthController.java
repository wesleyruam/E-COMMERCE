...
public class AuthController {
    ...
    @PostMapping("/refresh")
    public ResponseEntity<ServiceResponse<LoginResponseDTO>> refresh(@RequestBody @Valid RefreshTokenRequestDTO request) {
        ServiceResponse<LoginResponseDTO> response = authService.refresh(request);
        return ResponseEntity.status(response.getStatusCode()).body(response);
    }
}
